...
def ip_search_on_column_basis(data_frame, matched):
    ...
    matched_columns = [{f: 90, 'match': 'IP', 'basis' : 'column_name'} for f in column_headers if f in ip_sensitive_column_headers]
...
def ip_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 50:
            ...
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'IP', 'basis': 'column_data'})
...
def mac_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'MAC', 'basis' : 'column_name'}  for f in column_headers if f in mac_sensitive_column_headers]
    return matched_columns
...
def mac_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 50:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'MAC', 'basis': 'column_data'})
...
def msisdn_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'MSISDN', 'basis' : 'column_name'}  for f in column_headers if f in msisdn_sensitive_column_headers]
    return matched_columns

def imsi_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'IMSI', 'basis' : 'column_name'}  for f in column_headers if f in imsi_sensitive_column_headers]
    return matched_columns

def guid_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'GUID', 'basis' : 'column_name'}  for f in column_headers if f in guid_sensitive_column_headers]
    return matched_columns
...
def guid_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 50:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'GUID', 'basis': 'column_data'})
...
def hardware_serial_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Hardware_Serial', 'basis' : 'column_name'}  for f in column_headers if f in hardware_serial_sensitive_column_headers]
    return matched_columns
...
def hardware_serial_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 50:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Hardware_Serial', 'basis': 'column_data'})
