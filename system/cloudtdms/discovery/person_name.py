...
def search_on_column_basis(data_frame):
    ...
    matched_columns = [{f: 90, 'match': 'Name', 'basis' : 'column_name'} for f in column_headers if f in sensitive_column_headers]
...
def search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if len(f_intersection) > 100 or len(l_intersection) > 100:
            ...
            if score > 5:
                statistic_match.append({column: int(score),  'match': 'Name', 'basis': 'column_data'})
