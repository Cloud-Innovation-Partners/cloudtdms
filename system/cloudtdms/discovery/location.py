...
def latitude_search_on_column_basis(data_frame, matched):
    ...
    matched_columns = [{f: 90, 'match': 'Latitude', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in latitude_sensitive_column_headers]
    return matched_columns

def longitude_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Longitude', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in longitude_sensitive_column_headers]
    return matched_columns
...
def country_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Country', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in country_sensitive_column_headers]
    return matched_columns
...
def city_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'City', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in city_sensitive_column_headers]
    return matched_columns
...
def municipality_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Municipality', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in municipality_sensitive_column_headers]
    return matched_columns
...
def state_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'State', 'sensitvity': 'high', 'basis' : 'column_name'}  for f in column_headers if f in state_sensitive_column_headers]
    return matched_columns
