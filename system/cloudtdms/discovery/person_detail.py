...
def age_search_on_column_basis(data_frame, matched):
    ...
    matched_columns = [{f: 90,  'match': 'Age', 'basis' : 'column_name'} for f in column_headers if f in age_sensitive_column_headers]
    return matched_columns


def gender_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90,  'match': 'Gender', 'basis' : 'column_name'} for f in column_headers if f in gender_sensitive_column_headers]
    return matched_columns


def email_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90,  'match': 'Email', 'basis' : 'column_name'} for f in column_headers if f in email_sensitive_column_headers]
    return matched_columns

def dob_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Date of Birth', 'basis' : 'column_name'} for f in column_headers if f in dob_sensitive_column_headers]
    return matched_columns

def cc_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Credit Card', 'basis' : 'column_name'} for f in column_headers if f in credit_card_sensitive_column_headers]
    return matched_columns

def ssn_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Social Security Number', 'basis' : 'column_name'} for f in column_headers if f in ssn_sensitive_column_headers]
    return matched_columns

def blood_group_search_on_column_basis(data_frame, matched):
    column_headers = data_frame.columns
    matched_columns = [{f: 90, 'match': 'Blood Group', 'basis' : 'column_name'} for f in column_headers if f in blood_group_sensitive_column_headers]
    return matched_columns
...
def age_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if len(age_intersection) >= 63:
            ...
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Age', 'basis' : 'column_data'})
...
def email_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 100:
            ...
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Email', 'basis' : 'column_data'})
...
def dob_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 100:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Date Of Birth', 'basis' : 'column_data'})
...
def cc_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 100:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Credit Card', 'basis' : 'column_data'})
...
def blood_group_search_on_data_basis(data_frame, matched):
    ...
    for column in columns:
        ...
        if sum > 100:
            score = (sum / len(data_frame)) * 100
            if score > 5:
                statistic_match.append({column: int(score), 'match': 'Blood Group', 'basis' : 'column_data'})
